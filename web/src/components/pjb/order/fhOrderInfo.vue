...
<script>
	export default {
		...
		methods: {
			...
			selectAll() {
				fetch(`/api/pjb/fhOrder/selAll?id=${this.id}&starttime=${this.starttime}&endtime=${this.endtime}&nickname=${this.nickname}&pageNum=${this.pageNum}`).then(function(e) {
					return e.text();
				}).then((e) => {
					this.pages = e;
				})
			...
			selectPage() {
				...
				fetch("/api/pjb/fhOrder/selPage?" + params).then(function(e) {
					return e.json();
				}).then((e) => {
					if(e.message != "err") {
						e.forEach((val) => {
