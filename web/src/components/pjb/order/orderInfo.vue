<template>
	<el-tabs v-model="activeName" type="card" @tab-click="handleClick">
		...
		<el-tab-pane label="收货订单" name="second">
			<div class="root">
				...
				<el-table :data="tableData" border style="width: 100%">
					...
					<el-table-column label="收货类型" width="100">
						<template slot-scope="scope">
							<span v-if="scope.row.type==0">租用</span>
							<span v-else-if="scope.row.type==1">退租</span>
							<span v-else-if="scope.row.type==2">购买</span>
							<span v-else-if="scope.row.type==4">维修</span>
						</template>
					</el-table-column>
					<el-table-column prop="time" label="订单时间" width="152">
					</el-table-column>
					<el-table-column prop="mid" label="收货机器号" width="100">
					</el-table-column>
					<el-table-column prop="express_no" label="快递单号" width="120">
					</el-table-column>
					<el-table-column prop="crt_time" label="收货时间" width="152">
					</el-table-column>
					<el-table-column label="状态" width="100">
						<template slot-scope="scope">
							<span v-if="scope.row.state==0">待收货</span>
							<span v-else-if="scope.row.state==1">已收货</span>
						</template>
					</el-table-column>
		...
		<el-tab-pane label="交易订单" name="three">
			<pjbJyOrderInfo></pjbJyOrderInfo>
		</el-tab-pane>
	</el-tabs>
</template>
...
<script>
    ...
    export default {
		...
		methods: {
			edit(item) {
				fetch(`/api/pjb/shOrder/edit?id=${item.id}`).then(function(e) {
					return e.text();
				}).then((e) => {
					if(e == "ok") {
						item.state = 1;
			...
			selectAll() {
				fetch(`/api/pjb/shOrder/selAll?id=${this.id}&starttime=${this.starttime}&endtime=${this.endtime}&nickname=${this.nickname}&pageNum=${this.pageNum}`).then(function(e) {
					return e.text();
				}).then((e) => {
					this.pages = e;
				})
			...
			selectPage() {
				...
				fetch("/api/pjb/shOrder/selPage?" + params).then(function(e) {
					return e.json();
				}).then((e) => {
					if(e.message != "err") {
						e.forEach((val) => {
